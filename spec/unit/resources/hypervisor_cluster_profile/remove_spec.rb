...
describe 'oneview_test::hypervisor_cluster_profile_remove' do
  ...
  it 'removes it when it exists' do
    ...
    expect_any_instance_of(base_sdk::HypervisorClusterProfile).to receive(:remove).and_return(true)
  ...
  it 'does nothing when it does not exist' do
    allow_any_instance_of(base_sdk::HypervisorClusterProfile).to receive(:retrieve!).and_return(false)
    expect_any_instance_of(base_sdk::HypervisorClusterProfile).to_not receive(:remove)
    expect(real_chef_run).to remove_oneview_hypervisor_cluster_profile('HypervisorClusterProfile1')
  end
end
